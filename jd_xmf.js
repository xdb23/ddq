...
$.JDLOGXYZ_TOKEN = process.env.JDLOGXYZ_TOKEN;
if (!$.JDLOGXYZ_TOKEN) {
    console.log("请在环境变量中填写JDLOGXYZ_TOKEN=你的token");
    exit(0);
}
$.UUID = randomString(40);
$.UA='jdapp;iPhone;10.2.0;13.1.2;'+$.UUID+';M/5.0;network/wifi;ADID/;model/iPhone8,1;addressid/2308460622;appBuild/167853;jdSupportDarkMode/0;Mozilla/5.0 (iPhone; CPU iPhone OS 13_1_2 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Mobile/15E148;supportJDSHWK/1;';
var logs;
!(async () => {
  await requireConfig();
  if (!cookiesArr[0]) {
...
async function doInteractiveAssignment(projectId, encryptAssignmentId, itemId, actionType) {
  ...
  let body = { "encryptProjectId": projectId, "encryptAssignmentId": encryptAssignmentId, "sourceCode": "acexinpin0823", "itemId": itemId, "actionType": actionType, "completionFlag": "", "ext": {},"extParam":{"businessData":{"random":`${log.random}`},"signStr":`${log.log}`,"sceneid":"XMFhPageh5"} }
  return new Promise(resolve => {
    $.post(taskPostUrl("doInteractiveAssignment", body, log.ua, log.cookie), async (err, resp, data) => {
    //$.log(data)
      try {
        if (err) {
          console.log(`${err}`)
...
function taskPostUrl(function_id, body, ua, ck) {
  let opts = {
...
async function getLog(pin) {
  return new Promise(async resolve => {
      $.get({
          url: `https://sodian.vip/xmf_log?token=${$.JDLOGXYZ_TOKEN}&pin=${pin}`
      }, (err, resp, data) => {
          try {
              data = JSON.parse(data)
          } catch (e) {
